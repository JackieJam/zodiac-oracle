...
class ForecastRuleEngine:
    ...
    BASE_SCORE = 60.0  # 基线：无明显星象影响
    FACTOR_DELTA = 12  # 每个因子的最大影响力
    SCORE_MIN = 25
    SCORE_MAX = 95

    def score(self, context: AstroContext) -> Forecast:
        raw_scores = {area: self.BASE_SCORE for area in AREAS}
        for factor in context.factors:
            area = THEME_TO_AREA.get(factor.theme, "career")
            raw_scores[area] += self._factor_delta(factor, self.FACTOR_DELTA)

        scores = {area: int(max(self.SCORE_MIN, min(self.SCORE_MAX, round(value)))) for area, value in raw_scores.items()}
    ...
    @staticmethod
    def _factor_delta(factor: AstroFactor, max_delta: float = 12) -> float:
        # weight: 0.2-1.0 (基于容许度与最大容许度的比值)
        # 支撑相位(拱/六合): 正向调整
        # 压力相位(刑/冲): 等量负向调整，不打折
        # 中性相位(合相): 轻微正向(取决于参与行星)
        base = factor.weight * max_delta
    ...
    def score_all_signs(self, period_range, astro_engine) -> dict[str, int]:
        """计算 12 个星座的整体分数，用于排名对比。"""
        from xingzuo.astro_engine import SIGN_ORDER
        results = {}
        for sign in SIGN_ORDER:
            context = astro_engine.public_context(sign, period_range)
            forecast = self.score(context)
            results[sign] = forecast.overall_score
        return results

    @staticmethod
    def compute_ranking(all_scores: dict[str, int], current_sign: str) -> dict:
        """根据全部星座分数计算排名和相对标签。"""
        sorted_signs = sorted(all_scores.items(), key=lambda x: x[1], reverse=True)
        rank_map = {sign: rank + 1 for rank, (sign, _) in enumerate(sorted_signs)}
        total = len(sorted_signs)
        rank = rank_map.get(current_sign, total // 2)
        score = all_scores.get(current_sign, 60)
        ...
        return {
            ...
            "tag": tag,
            "tag_type": tag_type,
            "top3": [{"sign": s, "score": sc} for s, sc in sorted_signs[:3]],
            "bottom3": [{"sign": s, "score": sc} for s, sc in sorted_signs[-3:]],
        }
