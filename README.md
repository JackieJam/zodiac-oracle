...
一个“西洋热带占星”的专业星座评测 Web 工具原型。用户可以先点击 12 星座查看今日/本周运势，也可以填写出生信息生成个人化行运报告。

## 功能

- 大众入口：`GET /api/horoscope/public?sign=aries&period=daily`
- 个人化入口：`POST /api/horoscope/personal`
- 追问入口：`POST /api/horoscope/ask`，基于当前报告回答用户问题
- 统一输出：总评、事业/关系/财富/身心评分、深度解读、重点提醒、行动建议、占星依据、免责声明
- 内容生成：默认使用规则模板；配置 `DEEPSEEK_API_KEY` 后会在不新增占星结论的前提下润色摘要，并为“问星盘”提供更自然的互动回答
...
1. 星历报告：先用 Swiss Ephemeris 计算真实行运行星和相位。
2. 今日切口：把最重要的相位翻译为盲区、最佳动作和微仪式。
3. 问星盘：用户可继续追问事业、关系、财富、身心等问题；有大模型时由 LLM 基于报告回答，没有时用规则兜底。
